...
pub async fn start(
    ...
    bot.send_message(msg.chat.id, ADD_TRAVELER_ASK_NAME.translate(ctx))
        .await?;
    dialogue
...
pub async fn receive_name(
    ...
    if text.is_empty() {
        ...
        bot.send_message(msg.chat.id, ADD_TRAVELER_INVALID_NAME.translate(ctx))
            .await?;
        return Ok(());
    }
    ...
    let name = match Name::from_str(text) {
        ...
        Err(err) => {
            ...
            let reply = format!(
                "{invalid}\n\n{reason}",
                invalid = ADD_TRAVELER_INVALID_NAME.translate(ctx.clone()),
    ...
    let cmd = Command::AddTraveler { name };

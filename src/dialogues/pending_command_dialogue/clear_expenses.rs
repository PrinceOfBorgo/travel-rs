//! `/clearexpenses` confirmation dialogue.
...
pub const CALLBACK_PREFIX: &str = "clrexp:";
pub const CONFIRM_CALLBACK: &str = "clrexp:__confirm__";
pub const DENY_CALLBACK: &str = "clrexp:__deny__";

// ─── State ───────────────────────────────────────────────────────────────────

#[derive(Debug, Clone)]
pub enum ClearExpensesState {
    Confirm,
}
...
async fn ask_confirmation(
    ...
    let prompt = i18n::dialogues::CLEAR_EXPENSES_CONFIRM.translate(ctx.clone());
    ...
    dialogue
        .update(PendingCommandState::ClearExpenses(
            ClearExpensesState::Confirm,
        ))
        .await?;
    Ok(())
}
...
pub async fn start(
    ...
    tracing::info!("Dialogue started: /clearexpenses");
    Ok(())
}
...
pub async fn receive_confirm_text(
    ...
    match parse_confirm_answer(text) {
        ...
        ConfirmAnswer::No => {
            dialogue.exit().await?;
            let process_name =
                i18n::commands::RUNNING_PROCESS_CLEAR_EXPENSES.translate(Arc::clone(&ctx));

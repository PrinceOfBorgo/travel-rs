//! `/clearall` confirmation dialogue.
...
async fn ask_confirmation(
    ...
    let prompt = i18n::dialogues::CLEAR_ALL_CONFIRM.translate(ctx.clone());
    ...
    dialogue
        .update(PendingCommandState::ClearAll(ClearAllState::Confirm))
        .await?;
    Ok(())
}
...
pub async fn start(
    ...
    tracing::info!("Dialogue started: /clearall");
    Ok(())
}
...
pub async fn receive_confirm_text(
    ...
    match parse_confirm_answer(text) {
        ...
        ConfirmAnswer::No => {
            dialogue.exit().await?;
            let process_name =
                i18n::commands::RUNNING_PROCESS_CLEAR_ALL.translate(Arc::clone(&ctx));
